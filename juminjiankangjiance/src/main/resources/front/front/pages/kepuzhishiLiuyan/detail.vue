	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.kepuzhishiLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('kepuzhishiLiuyan', this.kepuzhishiLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.kepuzhishiLiuyanPhoto ? this.detail.kepuzhishiLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('kepuzhishiLiuyanYuyue', {
									yonghuId: _this.user.id,
									kepuzhishiLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/kepuzhishiLiuyanYuyue/list');
							}
						}
					});
				},
