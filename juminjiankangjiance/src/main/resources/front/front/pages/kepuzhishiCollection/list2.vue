	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.kepuzhishiName" type="text" placeholder="科普"></input>
		</view>
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('kepuzhishiCollection','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
    ...
    <view @tap="onDetailTap(product)" class="listm flex flex-between"
        ...
        <view class="listmr" style="padding: 10rpx ;">
            ...
            <view style="display: flex;justify-content: space-between;">
                <text v-if="isAuth('kepuzhishiCollection','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>
                <text v-if="isAuth('kepuzhishiCollection','删除')" class="cuIcon-delete"
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 类型 字段 字典表查询方法
		  	let kepuzhishiCollectionTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'kepuzhishi_collection_types'
		  	});
		  	this.kepuzhishiCollectionTypesList = kepuzhishiCollectionTypes.data.list;
			kepuzhishiCollectionTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
            this.categoryList = kepuzhishiCollectionTypes.data.list
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`kepuzhishiCollection`, params);
			...
			onDetailTap(item) {
				this.$utils.jump(`../kepuzhishi/detail?id=${item.kepuzhishiId}`)
			},
			// 修改
			onUpdateTap(id) {
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`kepuzhishiCollection`, {
                                id:id,
                            });
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('kepuzhishiCollection', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.mescroll.resetUpScroll()
						}
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.kepuzhishiCollectionShenheTime=shijian;
                            await _this.$api.update(`kepuzhishiCollection`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.page(`kepuzhishiCollection`, searchForm);
				// 如果是第一页数据置空
