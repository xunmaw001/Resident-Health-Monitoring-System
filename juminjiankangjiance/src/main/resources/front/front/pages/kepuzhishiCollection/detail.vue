				...
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.kepuzhishiCollectionValue}}
							</view>
						</view>
					</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.kepuzhishiCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('kepuzhishiCollection', this.kepuzhishiCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.kepuzhishiCollectionPhoto ? this.detail.kepuzhishiCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('kepuzhishiCollectionYuyue', {
									yonghuId: _this.user.id,
									kepuzhishiCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/kepuzhishiCollectionYuyue/list');
							}
						}
					});
				},
