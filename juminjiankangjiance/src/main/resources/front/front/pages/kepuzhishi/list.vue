	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.kepuzhishiName" type="text" placeholder="标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('kepuzhishi','新增')" @click="onAddTap()"
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='kepuzhishiTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in kepuzhishiTypesList" :key="index" :class="kepuzhishiTypes===item.codeIndex?'tabActive':''"
                                  @tap="kepuzhishiTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
...
<view class="list"
    ...
    <view @tap="onDetailTap(product)" class="listm flex flex-between"
        ...
        <image
                ...
                :src="product.kepuzhishiPhoto?baseUrl+product.kepuzhishiPhoto.split(',')[0]:''"></image>
        <view class="listmr" style="padding: 10rpx ;">
            <view class="hide1"
                ...
                {{product.kepuzhishiName}}
            </view>
            <view class="hide1" v-if="product.kepuzhishiNewMoney"
                ...
                ￥{{product.kepuzhishiNewMoney}}
            </view>
            <view class="hide1" v-else-if="product.kepuzhishiValue"
                  :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","fontSize":"30rpx","lineHeight":"56rpx","borderStyle":"solid"}'>
                {{product.kepuzhishiValue}}
            </view>
            <view style="display: flex;justify-content: space-between;">
                <text v-if="isAuth('kepuzhishi','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>
                <text v-if="isAuth('kepuzhishi','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 科普类型 字段 字典表查询方法
            async kepuzhishiTypesSelect(){
                let kepuzhishiTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'kepuzhishi_types',
                });
                kepuzhishiTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.kepuzhishiTypesList = kepuzhishiTypes.data.list
            },
            //当前表的 科普类型字段类别搜索
			kepuzhishiTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.kepuzhishiTypes != 0) {
                    params.kepuzhishiTypes = this.kepuzhishiTypes
                }
                let _this = this
                let res = await this.$api.list(`kepuzhishi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('kepuzhishi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.kepuzhishiShenheTime=shijian;
                            await _this.$api.update(`kepuzhishi`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.kepuzhishiName) {
                    searchForm['kepuzhishiName'] = this.searchForm.kepuzhishiName
                }

                let res = await this.$api.list(`kepuzhishi`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.kepuzhishiContent!=null){
                        item.kepuzhishiContent = item.kepuzhishiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
