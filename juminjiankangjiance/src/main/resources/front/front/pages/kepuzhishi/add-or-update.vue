        ...
        <form class="app-update-pv">
            <view
                ...
                <input   disabled
                         ...
                         :disabled="ro.kepuzhishiName" type="text" v-model="ruleForm.kepuzhishiName" placeholder="标题"></input>
            </view>
                <view
                        ...
                        class="cu-form-group" @tap="kepuzhishiPhotoTap">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">封面</view>
                    <view class="right-input" style="padding:0;textAlign:left">
                        <image
                                ...
                                class="avator" v-if="ruleForm.kepuzhishiPhoto" :src="baseUrl+ruleForm.kepuzhishiPhoto" mode="aspectFill">
                        </image>
                      ...
                      class="cu-form-group">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">科普类型</view>
                    <picker @change="kepuzhishiTypesChange" :value="kepuzhishiTypesIndex" :range="kepuzhishiTypesOptions" range-key="indexName">
                        <view
                                :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
                                class="uni-input">{{ruleForm.kepuzhishiTypes?ruleForm.kepuzhishiValue:"请选择科普类型"}}</view>
                    </picker>
                </view>
                <view
                        :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(218, 220, 219, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"108rpx"}'
                        class="cu-form-group" @tap="kepuzhishiVideoTap">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">科普视频</view>
                    <view class="right-input" style="padding:0;textAlign:left">
                        <image
                                :style='{"width":"68rpx","boxShadow":"0 0 0px rgba(0,0,0,.3)","borderRadius":"100%","textAlign":"left","height":"68rpx"}'
                                class="avator" v-if="ruleForm.kepuzhishiVideo" :src="baseUrl+ruleForm.kepuzhishiVideo" mode="aspectFill">
                        </image>
                        <image  :style='{"width":"68rpx","boxShadow":"0 0 0px rgba(0,0,0,.3)","borderRadius":"100%","textAlign":"left","height":"68rpx"}'
                                class="avator" v-else src="../../static/gen/upload.png" mode="aspectFill"></image>
                    </view>
...
<!-- text后缀 -->
            <view
                ...
                <textarea :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"16rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.kepuzhishiContent" v-model="ruleForm.kepuzhishiContent" placeholder="科普介绍"/>
            </view>
...
<script>
    ...
    export default {
		...
		/*下拉框*/
			let kepuzhishiTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'kepuzhishi_types',
            }
			let kepuzhishiTypes = await this.$api.page(`dictionary`, kepuzhishiTypesParams);
			this.kepuzhishiTypesOptions = kepuzhishiTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`kepuzhishi`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.kepuzhishiId){
                this.ruleForm.kepuzhishiId = options.kepuzhishiId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.kepuzhishiName)) {
                    this.$utils.msg(`标题不能为空`);
                    return
                }
				if ((!this.ruleForm.kepuzhishiPhoto)) {
                ...
                if (this.ruleForm.id) {
                    await this.$api.update(`kepuzhishi`, this.ruleForm);
                } else {
                    await this.$api.save(`kepuzhishi`, this.ruleForm);
                }
