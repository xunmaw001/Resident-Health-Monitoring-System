			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.kepuzhishiName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
					...
					<view class="yinyuenei uni-padding-wrap uni-common-mt">
						<view>
							<video id="myVideo" style="preload:auto;margin: 20rpx;border-radius: 20rpx;" muted
								   :src="baseUrl+detail.kepuzhishiVideo" controls></video>
						</view>
					</view>
				</view>
				<!-- 简介 -->
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.kepuzhishiContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.kepuzhishiId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getkepuzhishiCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        kepuzhishiId: this.kepuzhishiId,
                        yonghuId: this.user.yonghuId,
						kepuzhishiLiuyanPingfenNumber: this.scoreData,
                        kepuzhishiLiuyanText: this.commentData,
                    }
                    await this.$api.save(`kepuzhishiLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('kepuzhishi', this.kepuzhishiId);
                    this.detail = res.data
					this.detail.kepuzhishiContent = this.detail.kepuzhishiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.kepuzhishiPhoto ? this.detail.kepuzhishiPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getkepuzhishiCollection() {
					let params = {
						page: 1,
						limit: 1,
						kepuzhishiId: this.kepuzhishiId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`kepuzhishiCollection`, params);
					if (res.data.list.length == 1) {
						let kepuzhishiCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('kepuzhishiCollection', JSON.stringify([kepuzhishiCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getkepuzhishiCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('kepuzhishiCollection', {
									yonghuId: _this.user.id,
									kepuzhishiId: _this.detail.id,
									kepuzhishiCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('kepuzhishiLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						kepuzhishiId: this.kepuzhishiId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('kepuzhishi', _this.detail);
									_this.$utils.msg('取消成功');
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('kepuzhishiYuyue', {
									yonghuId: _this.user.id,
									kepuzhishiId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/kepuzhishiYuyue/list');
							}
						}
					});
				},
