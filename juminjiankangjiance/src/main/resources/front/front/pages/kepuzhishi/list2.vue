	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('kepuzhishi','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
...
<script>
	export default {
		...
		async onShow() {
            ...
            this.categoryList = kepuzhishiTypes.data.list
				...
				let params = {
                    ...
                    kepuzhishiDelete: 1,
							...
							if (this.categoryName != 0) {
                                params.kepuzhishiTypes = this.categoryName
                            }
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`kepuzhishi`, {
                                id:id,
                            });
