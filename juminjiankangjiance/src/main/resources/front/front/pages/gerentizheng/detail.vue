	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.gerentizhengId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('gerentizheng', this.gerentizhengId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.gerentizhengPhoto ? this.detail.gerentizhengPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('gerentizhengYuyue', {
									yonghuId: _this.user.id,
									gerentizhengId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/gerentizhengYuyue/list');
							}
						}
					});
				},
